...
public final class TranslateCountries {
	...
	private static void printTranslation(Locale[] localesToTranslate) {
		final Locale[] translateInLocales = new Locale[] { Locale.ENGLISH, Locale.FRENCH, Locale.GERMAN,
				Locale.ITALIAN };

		LOGGER.info("Countries in locales (in that order): ");
		IntStream.range(0, translateInLocales.length)
				.forEach(x -> LOGGER.info(translateInLocales[x].getDisplayLanguage(Locale.ENGLISH) + " "));
		LOGGER.info("\n");

		IntStream.range(0, localesToTranslate.length).forEach(t -> IntStream.range(0, translateInLocales.length)
				.forEach(x -> LOGGER.info(localesToTranslate[t].getDisplayCountry(translateInLocales[x]) + "\n")));
	}
