...
public final class NameMatcher {
	...
	/**
	 * Regex used to get name. "_; " <- symbols that can be written between names
	 * (optionally).
	 */
	...
	public static String getNameFromFullName(String input) {
		...
		if (m.matches()) {
			...
			if (m.group(1).length() == m.group(TWO).length()) {
				final String lengthResult = String.format(
						"first and last name's lengths are the same (%d)%n" + "[!] found name = \"%s\"%n%n",
						m.group(1).length(), m.group(1));
				LOGGER.info(lengthResult);

				return m.group(1);
			} else {
				final String lengthResult = String.format(
						"first name length = %d, last name length = %d%n" + "no name was found%n%n",
						m.group(1).length(), m.group(TWO).length());
				LOGGER.info(lengthResult);
