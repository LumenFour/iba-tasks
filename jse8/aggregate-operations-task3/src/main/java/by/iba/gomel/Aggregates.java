...
public final class Aggregates {
	...
	public static String getBookWithMaxPages(List<Book> list) {
		final Optional<Book> bookWithMaxPages = list.stream().max((s1, s2) -> {
	...
	public static int getAmountOfBooksWithCondition(List<Book> list) {
		return (int) list.stream()
				.filter(e -> e.getNumberOfPagesOfBook() > ONE_HUNDRED && e.getYearOfPublishing() < TWO_THOUSANDS)
				.count();
	}
	...
	public static int getAmountOfDuplicates(List<Book> list) {
		return list.size() - list.stream()
				.collect(Collectors.toCollection(() -> new TreeSet<Book>(Comparator.comparing(Book::getTitleOfBook))))
				.size();
	}
}
