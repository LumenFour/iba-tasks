...
/**
 * Concurrency task 2. The work principles described in tests.
 */
public final class QuestionAsker implements Runnable {
	...
	/**
	 * Logic of thread. Because of Sonar: "NPath Complexity is 448 (max allowed is
	 * 200)."
	 * 
	...
	private void logic(int questionNumber) {
		...
		try {
			...
			while ((System.currentTimeMillis() - startTime) < MS_TO_WAIT) {
				// for console input:
				// (System.currentTimeMillis() - startTime) < MS_TO_WAIT &&
				// !bufferReader.ready()
