...
public final class AggregateOperationOnString {
	...
	/**
	 * Takes List of Strings and finds a string with largest number. ! String
	 * participates in comparison only if contains one and only number. I.e. "3d3d"
	...
	public static String findMaxInt(List<String> list) {
		final Optional<String> result = list.stream().filter(e -> e.matches("\\D*\\d+\\D*")).max((s1, s2) -> {
