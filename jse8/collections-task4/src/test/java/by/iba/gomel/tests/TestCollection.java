...
public class TestCollection {
	...
	public void testCollection() {
		// Creating collection
		List<AbstractAnimal> collection = new LinkedList<>(Arrays.asList(new Bird("Sparrow", 2, 40, "red"),
				new Bird("Owl", 5, 60, "blue"), new Fish("Sturgeon", 3, 20, "purple"), new Fish("Carp", 2, 10, "pink"),
				new Cat("Russian Blue", 3, 30, "blue"), new Cat("Bombay", 6, 25, "yellow")));
		...
		// so here we get one animal (at random position) with required size using
		// binary search:
		...
		collection.stream().sorted(comparatorBySize).filter(e -> e.getSize() == searchForSize)
				.collect(Collectors.toList()).forEach(e -> LOGGER.info(e.toString()));

		// 3) Delete elements
		String colourToDelete = "blue";
		LOGGER.info("--Delete animals with colour = " + colourToDelete + "\n\n");

		collection.stream().sorted(comparatorBySize).filter(e -> !e.getColour().equalsIgnoreCase(colourToDelete))
				.collect(Collectors.toList()).forEach(e -> LOGGER.info(e.toString()));
